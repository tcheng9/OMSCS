...
q3_normalized_P_F_given_B        = .0089
...
def q4_localize(colors, measurements, motions, sensor_right, p_move):
    ...
    def sense(p, measurement):
        # The following code is from https://gatech.instructure.com/courses/364682/pages/4-localization-program-answer?module_item_id=3773066
        ...
        for i in range(len(colors)):
            for j in range(len(colors[0])):
                ...
                # aux[i][j] = p[i][j] * (hit * sensor_right + ((1-sensor_right) * (1-hit)))

                if colors[i][j] == measurement:
                    #color sensed correctly
                    aux[i][j] = p[i][j]*sensor_right
                else:
                    aux[i][j] = p[i][j]*(1-sensor_right)
    ...
    def move(p, motions):
        ...
        for i in range(len(p)):
            for j in range(len(p[0])):
                aux[i][j] = (p_move * p[i-y % len(p)][j-x % len(p[i])]) + ((1-p_move) * p[i][j])
        return aux
        # end of code citation
