...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    #Code copied from https://gatech.instructure.com/courses/364682/pages/2-online-slam-answer?module_item_id=3773850
    ...
    for k in range(len(data)):
        ...
        #measurement update
        for i in range(len(measurement)):
            #m is the index of the landmark coordinate
            m = 2 * (1 + measurement[i][0]) #1 + landmark_id
            for b in range(2):
                Omega.value[b][b] += 1.0 / measurement_noise
                Omega.value[m+b][m+b] += 1.0 / measurement_noise
                Omega.value[b][m+b] += -1.0 / measurement_noise #offdiagonal
                Omega.value[m+b][b] += -1.0/measurement_noise #offdiagonal
                Xi.value[b][0] += -measurement[i][1+b] / measurement_noise #measurement_row then find Lx_n, Ly_n offset by 1 for landmark_id
                Xi.value[m+b][0] += measurement[i][1+b] / measurement_noise


                #NOTE: +/- is like going TO vs. going FROM start -> end or vice versa.
        #expand the information matrix and vector by 1 new position
        ...
        for b in range(4):
            '''
            Q:Why is it range of 4?
            A: 4 diagonals (x_0, x_0), (y_0, y_0), (x_1, x_1), (y_1,y_1)
            '''
            Omega.value[b][b] += 1.0 / motion_noise

        #now for offdiagonal -> which are harder to index
        for b in range(2):
        ...
        #now factor out previous pose and do a,b,c calcs
